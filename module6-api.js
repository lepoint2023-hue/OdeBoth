...
let exhaustedUntil  = parseInt(sessionStorage.getItem("ode_exhausted") || "0", 10);
...
/* Si le quota a expiré depuis la dernière session, nettoyer immédiatement */
if (exhaustedUntil && Date.now() >= exhaustedUntil) {
  exhaustedUntil = 0;
  sessionStorage.removeItem("ode_exhausted");
...
async function callGemini(userMessage) {
  ...
  try {
    ...
    /* Les deux saturés — on attend 5s puis retente Groq une fois
       (les quotas Groq se réinitialisent par minute) avant de déclarer indisponible */
    if (response.status === 429) {
      await new Promise(function(r){ setTimeout(r, 5000); });
      try {
        response = await _callProxy("groq", systemPrompt, history, controller.signal);
        activeProvider = "groq";
      } catch(e) {}
    }

    /* Si toujours 429 après retry → courte pause de 3 min */
    if (response && response.status === 429) {
      exhaustedUntil = Date.now() + 3 * 60 * 1000;
      activeProvider = "groq";
