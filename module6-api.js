...
const FETCH_TIMEOUT = 90000; // 90s — absorbe le délai Retry-After Groq (~60s max)
const MAX_TOKENS    = 1500; // augmenté — évite les réponses tronquées

let activeProvider  = sessionStorage.getItem("ode_provider") || "groq";
let exhaustedUntil  = 0; // jamais persisté — un rechargement de page repart toujours frais
/* Exposition sur window pour que checkApiStatus() dans index.html puisse le lire */
...
(function scheduleNightlyReset() {
  ...
  function doReset() {
    ...
    console.info("[Ode] Reset nocturne — retour sur Groq, quotas réinitialisés");
...
async function callGemini(userMessage) {
  /* Si les deux providers sont connus comme épuisés, ne pas appeler le proxy */
  ...
  try {
    ...
    /* ── Fonction : appel avec retry automatique sur 429 ── */
    async function tryCall(prov) {
      let resp = await _callProxy(prov, systemPrompt, history, controller.signal);
      if (resp.status !== 429) return resp;

      /* Lire le délai Retry-After (Groq envoie cette info) */
      const retryAfter = parseInt(resp.headers.get('Retry-After') || '0', 10);
      const waitMs     = Math.min((retryAfter > 0 ? retryAfter : 15) * 1000, 65000);
      console.info("[Ode] " + prov + " rate-limit — attente " + Math.round(waitMs/1000) + "s...");

      await new Promise(function(r){ setTimeout(r, waitMs); });
      return await _callProxy(prov, systemPrompt, history, controller.signal);
    }

    /* 1. Essai avec le provider actif */
    response = await tryCall(provider);

    /* 2. Toujours 429 → basculer sur l'autre provider */
    if (response.status === 429) {
      const next = (provider === "groq") ? "gemini" : "groq";
      console.info("[Ode] " + provider + " épuisé → bascule sur " + next);
      activeProvider = next;
      provider       = next;
      response       = await tryCall(provider);
    }

    /* 3. Les deux épuisés → blocage court (2 min, mémoire seulement — pas sessionStorage) */
    if (response && response.status === 429) {
      exhaustedUntil = Date.now() + 2 * 60 * 1000; // 2 min, pas de sessionStorage
      activeProvider = "groq";
      hideTyping();
