...
async function callGemini(userMessage) {
  ...
  if (Date.now() < exhaustedUntil) {
    window.addMsg("bot", MSG_QUOTA[getLang()]);
    return;
  }
  ...
  try {
    ...
    if (response.status === 429) {
      ...
      window.addMsg("bot", MSG_QUOTA[lang]);
      _finaliseCall();
    ...
    hideTyping();
    if (typeof setStatus === "function") setStatus("online");
    window.addMsg("bot", reply, safeGetQR());
    ...
    window.addMsg("bot", isTimeout ? MSG_TIMEOUT[lang] : MSG_CONNEXION[lang]);
  }
