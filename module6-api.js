/* ═══════════════════════════════════════════════════════════
   MODULE 6 — APPEL API GROQ + GEMINI (fallback cyclique)
   ...
   4. Si les deux sont saturés → message d'erreur
...
const GEMINI_KEY = "AIzaSyAX2KgTrEBPOq0-fG6bIXj6sEY-6sxEg-I"; // AIza...
...
/* ── Réinitialisation quotidienne à 1h00 ── */
(function scheduleNightlyReset() {
  ...
  function doReset() {
    ...
    scheduleNightlyReset(); // replanifier pour le lendemain
  }
...
/* ══════════════════════════════════════════════════════
   APPEL PRINCIPAL — cycle Groq → Gemini → Groq
   ══════════════════════════════════════════════════════ */
async function callGemini(userMessage) {
  ...
  try {
    ...
    let attempted = new Set(); // évite les boucles infinies

    /* Cycle : essaie jusqu'à 2 providers différents */
    while (attempted.size < 2) {
      ...
      if (response.status === 429) {
        /* Ce provider est saturé — basculer sur l'autre */
        const next = (provider === "groq") ? "gemini" : "groq";
        console.info("[Ode] " + provider + " saturé → bascule sur " + next);
        activeProvider = next;
        provider       = next;
        continue; // réessaie avec l'autre provider
      }

      break; // réponse OK ou erreur non-429
    }

    /* Les deux sont saturés */
    if (response.status === 429) {
      hideTyping();
