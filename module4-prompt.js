...
/* ✅ FIX: paramètre renommé en `locale` pour ne plus masquer la variable
   globale `lang` définie dans index.html.
   ✅ FIX: dépendance explicitement documentée — getSvcs() est défini dans index.html */
function buildPrompt(svcId, locale) {

  /* ✅ FIX: validation du svcId avant utilisation — évite d'envoyer un id
     inconnu brut au LLM si un appel invalide est effectué */
  const knownIds = ['population','urbanisme','finances','env','social','enfance',
                    'salles','cimetiere','events','epn','rh','autre'];
  ...
  /* ✅ FIX: toutes les apostrophes manquantes corrigées dans le texte du prompt
     (l'assistante, c'est, d'orthographe, d'identité, d'envoi, d'adresse, etc.)
     En template literal JS, ' n'a jamais besoin d'être échappée. */
...
- Jour de collecte des poubelles : demander d'abord "De quel village êtes-vous ?" puis donner la bonne réponse :
  * Lavacherie, Amberloup → LUNDI (ordures chaque semaine, PMC 1 lundi sur 2, papier ~1x/mois)
  * Tillet, Sainte-Ode centre, Sprimont → MARDI (ordures chaque semaine, PMC 1 mardi sur 2, papier ~1x/mois)
  * Aviscount, Tonny → MERCREDI (ordures chaque semaine, PMC 1 mercredi sur 2, papier ~1x/mois)
  * Pour les dates exactes PMC/papier : https://www.idelux.be ou +32 63 23 18 11
  * Si le citoyen mentionne un village non listé : orienter vers IDELUX +32 63 23 18 11
