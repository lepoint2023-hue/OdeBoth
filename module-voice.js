...
(function _watchMessages() {
  ...
  if (document.readyState === 'loading') {
   ...
   TTS — Web Speech Synthesis
   Découpage en phrases pour contourner le bug Chrome/Android
   qui coupe la synthèse vocale après ~15 secondes.
   ════════════════════════════════════════════════════════ */

/* Découpe un texte en phrases courtes (≤ 220 caractères) */
...
function speakText(rawText) {
  ...
  _ensureVoices(() => {
    ...
    function speakNext() {
      ...
      /* Workaround Chrome/Android : relancer toutes les 8s */
      if (/Android|Chrome/i.test(navigator.userAgent)) {
        ...
        utt.onend = () => { clearInterval(ticker); origEnd(); };
      }
