...
function startListening() {
  ...
  _recognition.onresult = async (event) => {
    ...
    /* Instruction vocale : réponse courte mais TOUJOURS complète — jamais de phrase coupée.
       Le citoyen voit sa vraie phrase dans le chat, le LLM reçoit la consigne en plus. */
    const voiceMsg = transcript
      + ' [MODE VOCAL : réponds en 2-3 phrases maximum, toujours des phrases complètes, jamais de liste, langage oral naturel]';

    if (typeof addMsg     === 'function') addMsg('user', transcript); // texte affiché = phrase originale
    if (typeof callGemini === 'function') await callGemini(voiceMsg); // envoi avec consigne
  };
